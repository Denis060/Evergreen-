...
if (!supabaseUrl || !supabaseAnonKey) {
  console.warn("⚠️  SUPABASE_URL or SUPABASE_ANON_KEY missing from .env — DB calls will fail");
}

// Anon client — used only for public read routes
const supabase = createClient(supabaseUrl, supabaseAnonKey || "placeholder");

// Service role client — bypasses RLS, used for all authenticated write operations
...
// ─── Protected Routes ──────────────────────────────────────────────────────
// All DB operations here use getServiceClient() (service role) to bypass RLS.
...
// ─── Local dev server (Vite middleware injected dynamically) ──────────────
...
if (process.env.NODE_ENV !== "production") {
  (async () => {
    ...
    app.listen(3000, "0.0.0.0", () => {
      console.log("Evergreen Pro TV server running on http://0.0.0.0:3000");
    });
  })();
}
